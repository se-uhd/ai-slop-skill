...
No other runtime dependencies. Two helpers reach the network: the reference check (`verify_references.py`, CrossRef and DBLP) and the trope-catalog fetch (`fetch_tropes.py`, tropes.fyi). Offline the reference check degrades cleanly and reports `unchecked-offline`, and the review still completes. The catalog fetch is not optional: it has one source and no bundled copy, so a failed fetch stops the run unless you pass a catalog file with `--tropes=<path>`. The fetch accepts only a body with the catalog's shape (a title and its trope headings) and prints the size, heading count, and content hash of what it accepted to stderr, so a changed catalog is visible. The catalog is third-party text that the skills hand to the model as rules to apply. A paper repository that needs a reproducible review keeps its own copy and passes it with `--tropes`. Smoke tests for the helpers are at `plugins/ai-slop/scripts/tests/run_smoke.py` and can be run with `python3 plugins/ai-slop/scripts/tests/run_smoke.py`.
...
Ground mode fills the grounding comments the review flags as missing, for LaTeX papers. Review *finds* the `\cite{}` calls missing a grounding comment, and ground *fills* them. Given a LaTeX root, the ground skill:
...
3. **Checks each quote against its source** with `scripts/check_quotes.py`, which re-reads the file or page the agent named and confirms the quote occurs in it. A quote it cannot find is downgraded to a `TODO verify -- unverified` stub, and a source it cannot read as text (a PDF) is handed back for the skill to check by hand.
