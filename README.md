...
Repo mode is the whole-codebase counterpart to review and diff modes. Instead of one document, it runs `scripts/scan_repo.py` to extract the repository's natural-language text (every Markdown, plain-text, and LaTeX file in full; the comments and doc-comments of the source and config files in most common languages, with the `COMMENT_SPECS`/`NAME_SPECS` tables in `scan_repo.py` as the authoritative list; and the commit messages), then scans that prose against the general rules and the trope catalog and writes `ai-slop-report.md` with findings grouped by file (`### <relpath>`) and by commit (`### commit <short-sha>`), plus a `**Repo scope:**` header line. Comment detection is string-aware, so a `//` or `#` inside a string literal is not mistaken for a comment.
