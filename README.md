...
The skill keeps four layers of writing rules (listed under Rule layers below) and fetches a general AI-trope catalog (named patterns such as negative parallelism, em-dash addiction, and rule-of-three groupings, each tagged with a status) at runtime from [tropes.fyi](https://tropes.fyi), the only source for it. The review modes and init always load the general layer, add the LaTeX layer when the input is LaTeX source (detected by `scripts/detect_scope.py`, and never in `review-repo`), add the scientific (research article) layer for LaTeX or when you pass `--scientific`, and add the STE layer when you pass `--ste`. So the same bundle reviews a Markdown blog draft, a non-LaTeX manuscript (with `--scientific`), or a full LaTeX paper.
...
No other runtime dependencies. Three helpers reach the network: the reference check (`verify_references.py`, CrossRef and DBLP), the grounding-quote check (`check_quotes.py`, for a quote with a URL as its source), and the trope-catalog fetch (`fetch_tropes.py`, tropes.fyi). Offline the reference check degrades cleanly and reports `unchecked-offline`, and the review still completes. The catalog fetch is required. It has one source and no bundled copy, so a failed fetch stops the run unless you pass a catalog file with `--tropes=<path>`. The fetch accepts only a body with the catalog's shape (a title and its trope headings) and prints the size, heading count, and content hash of what it accepted to stderr, so a changed catalog is visible. The catalog is third-party text that the skills hand to the model as rules to apply. A paper repository that needs a reproducible review keeps its own copy and passes it with `--tropes`. Smoke tests for the helpers are at `plugins/ai-slop/scripts/tests/run_smoke.py` and can be run with `python3 plugins/ai-slop/scripts/tests/run_smoke.py`.
...
Diff mode is a variant of `/ai-slop:review` for git-versioned documents. Instead of walking the full draft, it runs `git diff <base>` (default `HEAD`) and restricts the rule and trope checks to the lines you added or modified in `.tex`, `.md`, and `.txt` files. The LaTeX layers apply to the changed `.tex` files and the general layer (plus the scientific layer with `--scientific`) to the rest, so a code repository with one paper under a subdirectory still gets its Markdown changes reviewed. The output is the same `ai-slop-report.md` schema with one extra header line (`**Diff scope:** base=<ref>, files=<list>`), so `/ai-slop:revise` can apply the suggestions unchanged.
...
Ground mode fills the grounding comments that the review flags as missing, for LaTeX papers. Review *finds* the `\cite{}` calls missing a grounding comment, and ground *fills* them. Given a LaTeX root, the ground skill:
...
3. **Checks each quote against its source** with `scripts/check_quotes.py`, which re-reads the file or page that the agent named and confirms the quote occurs in it. A quote it cannot find is downgraded to a `TODO verify -- unverified` stub, and a source it cannot read as text (a PDF) is handed back for the skill to check by hand.
4. **Writes the comments back** with `scripts/insert_grounding.py`, inserting `% GROUNDING: <key> -- "<quote>"` after each ungrounded cite and replacing any `TODO verify` stub left by revise mode or an earlier run (idempotently, matching indentation).
...
Bump the version at the ten callsites listed in the Versioning section. The next rev is the highest existing rev for the month plus one, with the bare `YYYY-MM` tag as rev0. After committing the bump, create the matching tag (`git tag YYYY-MM_revN`) and push it. Every release commit gets one, and tags must stay ancestors of `main`. Never amend or rebase a commit that has already been tagged or pushed. Additional work is a new rev, not a re-cut of the released commit.
