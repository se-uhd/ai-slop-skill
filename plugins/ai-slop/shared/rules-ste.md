...
- **Apply the layer to all prose you write or edit** (`T.scope`). The rules cover every natural-language text written or edited in STE mode: documents, code comments, commit messages, reports, and the replies an assistant writes while it works under these rules. Quotations and literal strings are the exception (see `T.quotations` and `T.literals`).
- **Precedence over the other layers** (`T.precedence`). This layer adds to the general, scientific, and LaTeX layers and relaxes none of their rules. Where this layer and another layer set different limits for the same thing, the limit in this layer applies. **Keep most sentences short** (`T.short-sentences`) and **Vary sentence length** (`T.vary-length`) replace the word ranges of **Sentence length** (`G.sentence-length`), and **Use active voice** (`T.active-voice`) narrows the exceptions of **Prefer active voice** (`G.active-voice`).

## Sentences

- **Keep most sentences short** (`T.short-sentences`). Give each sentence one main idea. Put the subject and the main verb near the start, before any long opening phrase or clause. Split a sentence when the reader must hold several clauses in mind before the point arrives, as in "Because the parser, which the build loads before any test runs, accepts unterminated strings, the report can lose data." -> "The parser accepts unterminated strings, so the report can lose data. The build loads the parser before any test runs." A sentence of more than 25 words is too long. Split it at a clause boundary, or move a qualification into a sentence of its own. A quotation or a literal string counts as one word.
- **Vary sentence length** (`T.vary-length`). Put a short sentence next to a long one that builds an idea, for example a five-word sentence next to a 24-word one. Do not write several sentences of similar length in a row. Three consecutive sentences within 5 words of each other in length are a flag. The long sentence still stays within the limit of **Keep most sentences short** (`T.short-sentences`).
- **Use active voice** (`T.active-voice`). Make the actor the subject, as in "The script writes the report," not "The report is written by the script." Use the passive voice only when the actor is unknown. The exception is narrower than those of **Prefer active voice** (`G.active-voice`), which also allow the passive when the actor is irrelevant or when the passive reads better. In a research article, the actor of the study's actions is "we" (see `S.we`).
...
- **Leave quotations as written** (`T.quotations`). Do not paraphrase, shorten, or simplify quoted text, whether a quotation in running text, a block quotation, the quote in a `% GROUNDING` comment, or quoted program output. The rules apply to the sentence around a quotation, and the quotation counts as one word toward the sentence-length limit.
- **Leave literal strings as written** (`T.literals`). Code, commands, file names, identifiers, configuration keys, URLs, and error messages stay exactly as they are. Do not add an article inside a literal string or replace a noun in it with a verb. A literal string counts as one word toward the sentence-length limit.

## Editing existing text

- **Keep a sentence that a rewrite would weaken** (`T.keep-and-mark`). When you edit existing text and the STE rewrite of a sentence loses meaning, precision, or force, keep the original sentence and mark it with `[KEPT: reason]`, where the reason is a short explanation (`[KEPT: the condition needs both clauses]`). Keep only the sentence at stake and rewrite its neighbors as usual. A later STE review does not flag a marked sentence under this layer's rules, but the other layers still apply to it. The marker goes where the format allows:
  - In a reply, or in text returned to the user for review, put the marker directly after the sentence.
  - In Markdown or HTML, put it directly after the sentence inside a comment, as in `<!-- [KEPT: reason] -->`, so the rendered page does not show it.
  - In LaTeX, put it in a comment on its own line after the line on which the sentence ends, as in `% [KEPT: reason]`. A comment inside the line would hide the text after it.
  - In plain text and in code comments, put it directly after the sentence.

## Self-Check Before Presenting Text (STE)

Apply these in addition to the self-checks of the other layers in scope:

1. **Sentence length** (`T.short-sentences`, `T.quotations`, `T.literals`). Count the words in each sentence, with each quotation and literal string as one word. Split every sentence of more than 25 words.
2. **Length variation** (`T.vary-length`). If three consecutive sentences are within 5 words of each other in length, lengthen or shorten one of them. Check that each long sentence builds an idea and sits next to a short one.
...
11. **Kept sentences** (`T.keep-and-mark`). Check that each sentence kept because a rewrite would weaken it carries a `[KEPT: reason]` marker in the position its format takes. Remove a marker from a sentence that meets the rules as written.
