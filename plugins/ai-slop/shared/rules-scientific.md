...
publications. The rationale behind the rules that a reader might push back on
is in `rules-rationale.md`, which the skills do not load.
...
- **Use "we" consistently** (`S.we`). First-person plural throughout. A single-author paper uses "I" throughout, or the venue's convention where it prescribes one. Do not mix the two.
...
**Present tense for the paper, past tense for the study** (`S.paper-vs-study`). Statements about what the paper *is* or *does* (e.g., its contributions, definitions, scope, structure) take present tense, because the paper exists in the reader's hands now. Statements about empirical actions performed during the study take past tense. Both can appear in one contributions list without inconsistency:
...
The same distinction governs cited prior work. An action that a study performed (e.g., it *surveyed*, *found*, *measured*) takes past or present perfect, while a claim that its paper makes (e.g., it *argues*, *proposes*, *defines*) can stay present. Do not mix the two within one attribution clause: "they *surveyed* ... and *found* ...", not "they *survey* ... and *found* ...".
...
- **Italicize statistical symbols** (`S.italic-symbols`) that are Latin letters: *M*, *SD*, *t*, *F*, *p*, *n*, *N*, *d*, *r*, *R²*, *df*. Do not italicize Greek letters (e.g., α, β, χ²) or abbreviations that are not variables (e.g., ANOVA, CI, OR).
...
6. **Verb tense consistency** (`S.tense-by-section`, `S.paper-vs-study`). Check the tense of every clause with a citation or author name as its subject. An empirical action that a cited study performed takes past or present perfect, not present. Do not judge a section correct from its dominant tense alone. A present-tense clause about prior work can appear in a section that is otherwise correct. Fix unmotivated tense shifts within a paragraph.
