...
tropes.fyi viewer and the upstream Gist are both unreachable. This script
...
overwrite would be a meaningless no-op):
  1. tropes.fyi viewer (markdown unwrapped from the rendered page)
  2. Upstream Gist (raw markdown)
...
`source: tropes.fyi; snapshot updated`); stdout is left empty.
...
from fetch_tropes import SOURCES, extract_markdown, try_fetch
