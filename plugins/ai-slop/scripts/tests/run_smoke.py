...
RULE_LAYERS = ('rules-general.md', 'rules-scientific.md', 'rules-latex.md', 'rules-ste.md')


def test_rule_layers_exist():
    """The rules ship as four layered files plus a rationale doc. The old
...
SENTENCE_LENGTH_FIXTURE = (
    'The reviewer reads each changed paragraph in the new file and checks every sentence '
    'against the rules before it writes the report to the working directory today.\n'   # 27 words
    '\n'
    'The scanner reads one file first. It then splits the text into sentences. '
    'Each sentence gets a count of its words.\n'                                        # 6/7/8, a run
    '\n'
    'It works well. The second sentence is much longer because it has to carry the reason '
    'and the example that the reader needs. Then it stops.\n'                           # 3/20/3, varied
    '\n'
    'The rule says "keep every sentence short, give each one main idea, and put the subject '
    'and the verb near the start of it" today.\n'                                      # the quote is one word
    '\n'
    'See e.g. Smith et al. (2020) and Fig. 2 for the method, which the authors describe in the '
    'appendix of the original article and in the replication package online.\n'       # 29 words, one sentence
)


def test_scan_sentences_counts_long_sentences_and_uniform_runs():
    with tempfile.TemporaryDirectory() as d:
        p = Path(d) / 'doc.md'
        write(p, SENTENCE_LENGTH_FIXTURE)
        rc, out, err = run('scan_sentences.py', str(p))
        assert rc == 0, f"sentences: rc={rc} err={err!r}"
        rows = [l.split('\t') for l in out.split('\n') if l]
        long_rows = [(r[0], r[2].split(' | ')[0]) for r in rows if r[1] == 'long-sentence']
        assert long_rows == [(f"{p}:1:1", '27 words'), (f"{p}:9:1", '29 words')], \
            f"sentences: long rows (an abbreviation must not split the last sentence): {long_rows!r}"
        runs = [(r[0], r[2].split(' | ')[0]) for r in rows if r[1] == 'uniform-run']
        assert runs == [(f"{p}:3:1", '6/7/8 words')], f"sentences: uniform runs {runs!r}"
        for token in ('9 sentence(s)', 'long-sentence=2', 'uniform-run=1'):
            assert token in err, f"sentences: summary missing {token!r}: {err!r}"


def test_scan_sentences_lists_passives_and_nominalizations():
    fixture = (
        'The report is generated by the script. The file was not found. The draft is being reviewed.\n'
        '\n'
        'We make a decision after the review. The team performed an analysis of the logs.\n'
        '\n'
        'The build is ready. We need more tests. This makes the configuration easier. '
        'The system provides an environment for tests. Do the tests pass? It is indeed true.\n'
    )
    with tempfile.TemporaryDirectory() as d:
        p = Path(d) / 'doc.md'
        write(p, fixture)
        rc, out, err = run('scan_sentences.py', str(p))
        assert rc == 0, f"patterns: rc={rc} err={err!r}"
        rows = [l.split('\t') for l in out.split('\n') if l]
        passive = [r[2].split(' | ')[0] for r in rows if r[1] == 'passive']
        assert passive == ['is generated', 'was not found', 'being reviewed'], f"patterns: passive {passive!r}"
        nominal = [r[2].split(' | ')[0] for r in rows if r[1] == 'nominalization']
        assert nominal == ['make a decision', 'performed an analysis'], f"patterns: nominalization {nominal!r}"
        # "is ready", "need", a causative "makes the", a noun that names a thing,
        # an auxiliary "Do", and "is indeed" are not listed.
        assert not any(r[0].startswith(f"{p}:5:") and r[1] in ('passive', 'nominalization') for r in rows), \
            f"patterns: false positive on line 5: {rows!r}"


def test_scan_sentences_skips_non_prose():
    md = ('---\ntitle: The title is generated by the tool\n---\n\n'
          '# A heading is written here and is never counted as a sentence by the scan at all today\n\n'
          'Intro text with `a long inline code span that has many words inside it and keeps on going '
          'for a while now` is fine.\n\n'
          '```python\nx = "The value is computed by the function."\n```\n\n'
          '> A block quote is generated by someone.\n\n'
          '| a | the table row is generated |\n\n'
          '- A list item is written here.\n'
          '- Another item.\n\n'
          'The data is fine. <!-- the passive is written in a comment -->\n')
    tex = ('\\documentclass{article}\nThe preamble is ignored by the scan.\n\\begin{document}\n'
           '\\section{A section title is skipped}\n'
           'We ran the tool on each file~\\cite{a,b,c}, as described by Smith et al.~\\cite{s} in Fig.~2 '
           'of the paper and in the appendix of the original article today. % a note\n'
           '% a comment line is generated here\n'
           '\\begin{verbatim}\nThe code is generated here.\n\\end{verbatim}\n'
           'The value is fine.\n% the value is stored in a comment\n'
           "We quote ``the output is written to disk.'' Then we stop.\n"
           '\\end{document}\nThe epilogue is ignored.\n')
    with tempfile.TemporaryDirectory() as d:
        pm, pt = Path(d) / 'doc.md', Path(d) / 'paper.tex'
        write(pm, md)
        write(pt, tex)
        rc, out, err = run('scan_sentences.py', str(pm))
        assert rc == 0, f"skips md: rc={rc} err={err!r}"
        rows = [tuple(l.split('\t')[:2]) for l in out.split('\n') if l]
        # Front matter, heading, code span, fence, block quote, table, and an
        # inline HTML comment add nothing. The list item stays.
        assert rows == [(f"{pm}:17:15", 'passive')], f"skips md: {rows!r}"
        rc, out, err = run('scan_sentences.py', str(pt))
        assert rc == 0, f"skips tex: rc={rc} err={err!r}"
        rows = [tuple(l.split('\t')[:2]) for l in out.split('\n') if l]
        # The citations and "et al." / "Fig." leave one 28-word sentence. The
        # preamble, comments, verbatim block, quotation, and epilogue list no
        # passive.
        assert (f"{pt}:5:1", 'long-sentence') in rows, f"skips tex: {rows!r}"
        assert not any(kind == 'passive' for _, kind in rows), f"skips tex: passive listed: {rows!r}"


def test_scan_sentences_usage_and_unreadable_paths():
    rc, out, err = run('scan_sentences.py')
    assert rc == 2 and 'usage' in err, f"no args: rc={rc} err={err!r}"
    with tempfile.TemporaryDirectory() as d:
        rc, out, err = run('scan_sentences.py', str(Path(d) / 'missing.md'))
        assert rc == 2 and 'none of the 1 path(s)' in err, f"unreadable: rc={rc} err={err!r}"
        p = Path(d) / 'doc.md'
        write(p, 'One sentence.\n')
        rc, out, err = run('scan_sentences.py', str(p), str(Path(d) / 'gone.md'))
        assert rc == 0 and 'scanned 1 file(s)' in err, f"partial: rc={rc} err={err!r}"


# ---------- rule keys ----------

LAYERS = ('rules-general.md', 'rules-scientific.md', 'rules-latex.md', 'rules-ste.md')
LAYER_PREFIX = {'rules-general.md': 'G', 'rules-scientific.md': 'S', 'rules-latex.md': 'L', 'rules-ste.md': 'T'}
...
EXPORT_CATALOG = """# AI Writing Tropes to Avoid

Add this file to your AI assistant's system prompt.
...
def test_export_rules_writes_general_layer_and_catalog():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        out = Path(d) / 'rules' / 'ai-slop.md'
        rc, _, err = run('export_rules.py', str(out), f'--tropes={catalog}')
        assert rc == 0 and err.startswith('created '), (rc, err)
        text = out.read_text(encoding='utf-8')
        assert text.startswith('# Writing rules\n'), text[:80]
        assert 'replies in chat' in text and 'commit messages' in text, text[:600]
        assert 'generated by `/ai-slop:export`' in text, text[:900]
...
def test_export_rules_replaces_only_an_earlier_export():
    """A second export replaces the first without --force. A file that is not
    an export is left alone (exit 3) until --force is passed."""
    with tempfile.TemporaryDirectory() as d:
        catalog = Path(d) / 'tropes.md'
        write(catalog, EXPORT_CATALOG)
        out = Path(d) / 'ai-slop.md'
        rc, _, err = run('export_rules.py', str(out), f'--tropes={catalog}')
        assert rc == 0, (rc, err)
        rc, _, err = run('export_rules.py', str(out), f'--tropes={catalog}')
        assert rc == 0 and err.startswith('replaced '), (rc, err)

        own = Path(d) / 'own.md'
        write(own, '# My rules\n\nWrite short sentences.\n')
        rc, _, err = run('export_rules.py', str(own), f'--tropes={catalog}')
        assert rc == 3 and 'not an earlier export' in err, (rc, err)
        assert own.read_text(encoding='utf-8').startswith('# My rules'), "own file changed"
        rc, _, err = run('export_rules.py', str(own), f'--tropes={catalog}', '--force')
        assert rc == 0 and err.startswith('replaced '), (rc, err)


def test_export_rules_usage_and_bad_catalog():
    """Usage errors and unreadable catalogs exit 2, the format layers are no
    option, and a catalog without trope headings exits 1 and writes nothing."""
    with tempfile.TemporaryDirectory() as d:
        out = Path(d) / 'ai-slop.md'
        rc, _, err = run('export_rules.py')
        assert rc == 2 and 'usage' in err, (rc, err)
        for flag in ('--latex', '--scientific'):
            rc, _, err = run('export_rules.py', str(out), flag)
            assert rc == 2 and 'usage' in err, (flag, rc, err)
        rc, _, err = run('export_rules.py', str(out), '--tropes=/nonexistent/tropes.md')
        assert rc == 2 and 'cannot read' in err, (rc, err)
        bad = Path(d) / 'bad.md'
        write(bad, '# Not a catalog\n\nNo trope headings here.\n')
        rc, _, err = run('export_rules.py', str(out), f'--tropes={bad}')
        assert rc == 1 and 'no `## ` trope heading' in err, (rc, err)
        assert not out.exists(), "a failed export wrote the file"
...
TESTS = [
    ...
    test_scan_sentences_counts_long_sentences_and_uniform_runs,
    test_scan_sentences_lists_passives_and_nominalizations,
