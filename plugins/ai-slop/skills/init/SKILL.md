...
3. **Load the AI-trope catalog.** Run `python3 ${CLAUDE_SKILL_DIR}/../../scripts/fetch_tropes.py ${CLAUDE_SKILL_DIR}/../../shared/tropes-snapshot.md`. The script tries the tropes.fyi viewer, then the upstream Gist, then the bundled fallback, and always emits a non-empty catalog body on stdout. It also prints one line to stderr identifying which source was used (e.g., `source: tropes.fyi`, `source: gist`, `source: bundled`). Capture that line for the summary in step 9.
...
5. **Build the WRITING.md content.** Compose the file as `<header>` + `<rules body>` + `<separator and trope intro>` + `<normalized trope body>`:
    ...
    These rules apply to all prose in this repository. They were generated by `/ai-slop:init` from the [ai-slop-skill](https://github.com/se-uhd/ai-slop-skill) (skill version 2026-09_rev14) and combine the writing rules maintained by the [Software Engineering Group at Heidelberg University](https://github.com/se-uhd) with a general AI-trope catalog from [tropes.fyi](https://tropes.fyi) by [ossama.is](https://ossama.is).
...
9. **Print a summary.** Tell the user, in three lines, what happened to each file: WRITING.md (created / overwritten / left alone / declined; include the trope source, that is, the tropes.fyi viewer, the upstream Gist, or the bundled snapshot), CLAUDE.md (created / appended / left alone because already referenced).
