...
- **Paper.** Auto-detected by running `python3 ${CLAUDE_SKILL_DIR}/../../scripts/find_latex_root.py`. Exit 0 means the printed path is the paper. Exit 2 means multiple candidates were printed, so ask the user. Exit 1 means no `.tex` root, so stop. PDF input is not supported, since revise mode edits LaTeX directly.
