...
This skill reviews the natural-language text spread across a whole repository, rather than a single document (`/ai-slop:review`) or the changed lines of one (`/ai-slop:review-diff`). It extracts every Markdown and plain-text file in full, the comments and doc-comments of the source and config files, and the repository's commit messages, then scans that prose against the general rules and the AI-trope catalog and writes an `ai-slop-report.md` grouped by file (and by commit). It is the right mode when a codebase's prose has drifted over many commits: a British spelling, an em-dash, or a trope sitting in a committed comment or a commit message that a diff review never revisits.
...
- **Code is not prose.** The extractor is heuristic. If a flagged line is code that it surfaced by accident, drop the finding rather than rewriting code.
