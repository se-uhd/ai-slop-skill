...
2. **Extract the citations.** Run `python3 ${CLAUDE_SKILL_DIR}/../../scripts/extract_cites.py <path>` and capture stdout to `grounding-cites.json` in the working directory. The JSON holds:
   ...
   The script prints to stderr a one-line summary (sites, unique keys, groundable sites still missing a comment, metadata coverage). Add `grounding-cites.json` and `grounding-quotes.json` to the repo's `.gitignore` (creating it if absent). Both files are generated artifacts and must not be committed.
...
The grounding fan-out is an **embedded-data + chunked-`parallel()`** workflow. The per-source claim bundles are embedded directly in the script (no separate file), each source is one schema-validated agent, and the agents run in fixed-size slices to stay under rate limits. Author it inline, substituting the bundles from `by_key` / `meta`:
...
Map each result to `grounding-quotes.json`: `status: 'quote'` becomes `{quote}`, and `status: 'todo'` becomes `{todo: reason}`. If the Workflow tool is unavailable in the client, fan the same per-source prompts out with individual sub-agents, or process them sequentially. The schema and the anti-fabrication rule are unchanged.
...
- **Never fabricate a quote.** A quote is written only when the source was retrieved and the text was copied from it. Otherwise it is `TODO verify -- <reason>`. This rule has no exceptions.
