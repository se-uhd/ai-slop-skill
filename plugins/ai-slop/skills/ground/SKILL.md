...
This skill fills the grounding comments that review mode flags as missing. `/ai-slop:review` lists every `\cite{}` missing a `% GROUNDING:` comment as a grounding to-do but never fills it. Ground mode fetches each cited source, extracts a verbatim quote supporting the claim that the paper attributes to it, and writes the grounding comment into the LaTeX. Where the source cannot be retrieved, it writes a `TODO verify -- <reason>` stub instead, never a quote from memory. Quote-less `TODO verify` stubs, left by `/ai-slop:revise` or by an earlier ground run, count as unfilled. Ground picks up the citations that carry these stubs and replaces the stub with the retrieved quote.
...
8. **Summarize.** Tell the user, in plain terms:
   ...
   - How many got a TODO, broken down by reason (`unverified` marks a returned quote that the check could not find in its source).
...
The grounding fan-out is a chunked-`parallel()` workflow. The per-source claim bundles are passed in as `args.sources` (built from `by_key` / `meta`). Each source gets one schema-validated agent, and the agents run in fixed-size slices to stay under rate limits. Author it inline:
...
- `../../scripts/extract_cites.py`: resolves the LaTeX root, follows `\input` / `\include`, and emits the `sites` / `by_key` / `meta` JSON. Shares its citation scanner with `find_citation_issues.py` (`cite_scan.py`) and its `.bib` parser with `check_bib_fields.py` / `verify_references.py` (`bib_parse.py`).
- `../../scripts/check_quotes.py`: checks each returned quote against the source that it names and downgrades each quote that it cannot find.
- `../../scripts/insert_grounding.py`: writes the `% GROUNDING:` comments back idempotently from the quotes JSON.
- `../../scripts/find_latex_root.py`, `../../scripts/detect_scope.py`: locate the root and restrict the skill to LaTeX.
...
- **Never fabricate a quote.** A quote is written only when the source was retrieved and the text was copied from it, and step 6 checks each quote against its source before insertion. Otherwise the comment is `TODO verify -- <reason>`. This rule has no exceptions.
