...
The skill's workflow is in `skills/review-diff/SKILL.md`. It runs `git diff` against a base ref (default `HEAD`), restricts the rule and trope checks to lines added or modified in `.tex`, `.md`, and `.txt` files, and writes `ai-slop-report.md` in the working directory using the same schema as `/ai-slop:review` so `/ai-slop:revise` can apply the suggestions unchanged. The rule layers are chosen per changed file via `scripts/detect_scope.py`. A `.tex` file gets all three layers. Any other file gets the general layer. Add `--scientific` to also apply the research-article rules to a non-LaTeX manuscript, and `--ste` to apply the Simplified Technical English layer to every changed file.
