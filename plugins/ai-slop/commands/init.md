...
The skill builds a `WRITING.md` file in the working directory by concatenating the bundled writing rules (`shared/rules-general.md`, `shared/rules-scientific.md`, and `shared/rules-latex.md`, auto-detected from the target directory via `scripts/detect_scope.py`: all three for a LaTeX project, the general layer for any other project, or general + scientific with `--scientific`) with the AI-trope catalog (fetched live from the tropes.fyi viewer, falling back to the upstream Gist and then the bundled `shared/tropes-snapshot.md`), and either creates a `CLAUDE.md` that references it or appends a reference to an existing one. The result is a repository where any Agent Skills client (e.g., Claude Code, Cursor, Copilot, Codex, Gemini CLI, JetBrains Junie) sees both the rules and the trope catalog through the standard CLAUDE.md mechanism, even if the user has not installed this plugin and even when offline.
