---
description: Review a whole repository's natural-language text for AI slop and rule violations, covering every Markdown and plain-text file, the comments in source and config, and the commit messages, using the general rules and the same report schema as `/ai-slop:review`. Generates a structured Markdown report grouped by file.
---
