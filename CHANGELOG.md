...
- **Added:** **Announce a count only when the reader tracks it** (`G.no-announced-counts`) in the general layer, from the v2 catalog's Compulsive counting and Preamble entries. The count stays where the reader checks items against it (a specification, a checklist, a study's conditions or research questions, a contribution list) and goes where continuous prose follows it. The test is a later sentence that refers back to the number.
- **Added:** **No enumerated list disguised as paragraphs** (`G.no-prose-listicle`), from the Excessive enumeration entry. The catalog names the shape a model produces once it is told to stop emitting bullet lists, which is also the shape **No list-cramming in a single sentence** (`G.no-list-cramming`) prescribes as a fix, so the new rule sets the boundary at scale: ordinals inside one paragraph are structure, ordinals opening consecutive paragraphs are an unmarked list.
- **Added:** a **Phrases to Avoid** entry for **"famously" / "notoriously" / "as is well known" / "a classic"**, from the Appeal to familiarity entry. The phrase borrows consensus without evidence and marks the claim that most needs a citation.
- **Changed:** eight rules tightened against the v2 entries they now cover. `G.no-formulaic-openings` takes the announce-then-answer opener, `G.no-metacommentary` takes sentences that narrate the writing's own reasoning, `G.no-coinages` takes the abstract label built from a domain word plus a problem-noun ("supervision paradox"), `G.plain-words` takes "lives in", `G.paragraph-length` takes the bare fragment set as a paragraph, `G.paragraph-padding` takes the aphorism shaped for quoting, `G.no-excess-bold` takes bold-first bullets, and `G.em-dash-glyphs` states that rewriting the glyph as `--` is not a fix for the dash. `S.cite-specific-works` gains the rule that a quantity word has to match the citations behind it.
- **Changed:** the bundle's own prose no longer frames a file or module as a place something inhabits ("the skill's workflow lives in ..." became "is in ..."), following the rule added to `G.plain-words`. Self-check items 9, 13, and 21 carry the new patterns, and items 26 and 27 cover the two new rules.
